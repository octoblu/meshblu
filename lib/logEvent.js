...
module.exports = function(eventCode, data) {
  ...
  if(config.log){
	...
	if (config.splunk && config.splunk.indexObj) {
		...
		tmpData = { "uuid": uuid, "timestamp": data.timestamp, "data": data };
		config.splunk.indexObj.submitEvent(tmpData, { sourcetype: "meshblu" }, function(err, result, config.splunk.indexObj) {
