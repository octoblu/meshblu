...
var mqttServer = function(config, parentConnection){
  ...
  server.on('published', function(packet, client) {
    try{
      ...
      else if('generateSessionId' === packet.topic){
        msg = JSON.parse(packet.payload.toString());
        if(msg.ack){
          ack = msg.ack;
          delete msg.ack;
          generateSessionId(msg, function(err, result){
            serverAck(client.skynetDevice, ack, result);
            var message = {uuid: msg.uuid, sessionId: result.sessionId}
            emitToClient('generateSessionId', client.skynetDevice, message);
          });
        }
      }
