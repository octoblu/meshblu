...
var mqttServer = function(config, parentConnection){
  ...
  server.on('published', function(packet, client) {
    var msg, ack, payload, packetObj, topic, request;
    ...
    if('message' === topic || 'tb' === topic){
      ...
      updateFromClient(client.skynetDevice, msg, function(){
        sendConfigActivity(client.skynetDevice.uuid, emitToClient);
      });
    }
    else if('resetToken' === topic){
      resetToken(client.skynetDevice, msg, emitToClient, function(error, token){
        if(error != null){
      ...
      generateAndStoreToken(client.skynetDevice, msg.uuid, function(error, result){
        if(error != null){
          return emitToClientDirectly(client.skynetDevice.uuid, {topic: 'error', payload: error, _request: request});
        }
        var message = {topic: 'generateAndStoreToken', payload: {uuid: msg.uuid, token: result.token, _request: request}};
        emitToClientDirectly(client.skynetDevice.uuid, message);
      });
    }
