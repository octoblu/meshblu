...
var mqttServer = function(config, parentConnection){
  ...
  server.on('published', function(packet, client) {
    debug('published', packet, 'payload:', packet.payload.toString());
    try{
      var msg, ack, payload;
      if('message' === packet.topic || 'tb' === packet.topic){
        try {
          payload = JSON.parse(packet.payload.toString())
        } catch (e) {
          payload = {devices: ['*'], payload: packet.payload.toString()}
        }
      ...
      else if('generateAndStoreToken' === packet.topic){
        msg = JSON.parse(packet.payload.toString());
        if(msg.ack){
          ack = msg.ack;
          delete msg.ack;
