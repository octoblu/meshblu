...
module.exports = {
  setup: function (server) {
    ...
    server.get('/events/:uuid', function(req, res){
      ...
      require('./authDevice')(req.params.uuid, req.query.token, function(auth){
        if (auth.authenticate == true){
          require('./getEvents')(req.params.uuid, function(data){
            console.log(data);
            // io.sockets.in(req.params.uuid).emit('message', data)
          if(data.error){
            res.json(data.error.code, data);
    ...
    server.get('/subscribe/:uuid', function(req, res){
      ...
      require('./authDevice')(req.params.uuid, req.query.token, function(auth){
        if (auth.authenticate == true){
          ...
          require('./subscribe')(req.params.uuid)
            .pipe(foo)
            .pipe(res);
    ...
    server.get('/inboundsms', function(req, res){
      ...
      require('./getPhone')(toPhone, function(uuid){
        ...
        require('./logEvent')(301, eventData);
        if(eventData.error){
    ...
    server.post('/data/:uuid', function(req, res){
      ...
      require('./authDevice')(req.params.uuid, req.params.token, function(auth){
        if (auth.authenticate == true){
    
          delete req.params.token;
    
          req.params['ipAddress'] = req.connection.remoteAddress
          require('./logData')(req.params, function(data){
            console.log(data);
            // io.sockets.in(data.uuid).emit('message', data)
            if(data.error){
              res.json(data.error.code, data);
      ...
      require('./authDevice')(req.params.uuid, req.query.token, function(auth){
        if (auth.authenticate == true){
          if(req.query.stream){
            ...
            require('./getData')(req, function(data){
              console.log(data);
              if(data.error){
                res.json(data.error.code, data);
              } else {
