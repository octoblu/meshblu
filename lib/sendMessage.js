...
function createMessageSender(socketEmitter, mqttEmitter, parentConnection){
  ...
  function messageForward(toDevice, emitMsg){
    if (!toDevice.meshblu) {
      return
    }
    doMessageHooks(toDevice, toDevice.meshblu.messageHooks, emitMsg);
    doMessageForward(toDevice.meshblu.messageForward, emitMsg, toDevice.uuid, function(error, messages) {
      _.each(messages, function(msg){
        getDevice(msg.forwardTo, function(error, forwardDevice) {
          ...
          if(!forwardDevice) {
            console.error('sendMessage.js: forwardDevice not found');
            return;
          }
          sendMessage(forwardDevice.uuid, msg.message, msg.topic, toDevice.uuid, toDevice, [forwardDevice.uuid]);
        });
      });
    });
  }
