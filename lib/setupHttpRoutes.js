...
function authorizeRequest(req, res, callback){
  ...
  if(req.header('skynet_auth_uuid') && req.header('skynet_auth_token')){
    var authUuid = req.header('skynet_auth_uuid');
    var authToken = req.header('skynet_auth_token');
  } else {
    var authUuid = req.header('meshblu_auth_uuid');
    var authToken = req.header('meshblu_auth_token');    
  }
  authDevice(authUuid, authToken, function (auth) {
    ...
    if(uuid && uuid.length > 30){
      ...
      whoAmI(req.params.uuid, false, function(results){
        if(results.error){
          ...
          if(securityImpl.canRead(fromDevice, results)){
...
function setupHttpRoutes(server, skynet){
  ...
  server.post('/devices/micro', function(req, res){
    ...
    //dont bother with CORS, this isnt for brosers
    //res.setHeader('Access-Control-Allow-Origin','*');
    req.params.ipAddress = getIP(req);
    register(req.params, function(data){
      if(data.error){
