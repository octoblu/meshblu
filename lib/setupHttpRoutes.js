...
function setupHttpRoutes(server, skynet){
  ...
  var subscribeAndForwardWithToken = function(response, uuid, token) {
  ...
  var subscribeAndForward = function(askingDevice, response, uuid, token){
    uuid = uuid || askingDevice.uuid
    if (token) {
      return subscribeAndForwardWithToken(response, uuid, token);
    }
