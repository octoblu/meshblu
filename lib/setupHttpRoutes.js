...
function subscribeBroadcast(req, res, type, skynet){
  authorizeRequest(req, res, function(fromDevice){
    ...
    var uuid = req.merged_params.uuid;
    ...
    whoAmI(uuid, false, function(results){
      ...
      securityImpl.canReceive(fromDevice, results, function(error, permission){
        if(error || !permission){
          return errorResponse({error: "unauthorized access"}, res);
        }
...
function setupHttpRoutes(server, skynet){
  ...
  server.get('/devices/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('devices',req, fromDevice));
      getDevices(fromDevice, {uuid: req.merged_params.uuid}, false, function(data){
          if(data.error){
            errorResponse(data.error, res);
          }else{
            res.json(data);
  ...
  server.put('/claimdevice/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      claimDevice(fromDevice, req.merged_params, function(error, data){
        if(error){
  ...
  server.get('/devices/:uuid/publickey', function(req, res){
    getPublicKey(req.merged_params.uuid, function(error, publicKey){
      if(error){
  ...
  server.post('/devices/:uuid/token', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      resetToken(fromDevice, req.merged_params.uuid, skynet.emitToClient, function(error,token){
        if(error){
          errorResponse(error, res);
        }else{
          var uuid = req.merged_params.uuid || fromDevice.uuid;
          skynet.sendConfigActivity(uuid, skynet.emitToClient);
          res.json({uuid: uuid, token: token});
        }
      });
    });
  });
  ...
  server.post('/devices/:uuid/tokens', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      generateAndStoreToken(fromDevice, req.merged_params.uuid, function(error, result){
        if(error){
  ...
  server.delete('/devices/:uuid/tokens/:token', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      revokeToken(fromDevice, req.merged_params.uuid, req.merged_params.token, function(error, result){
        if(error){
          return errorResponse(error, res);
        }
  ...
  server.put('/devices/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('devices',req, fromDevice));
      updateFromClient(fromDevice, req.merged_params, function(result){
        if(result.error){
          errorResponse(result.error, res);
        }else{
          skynet.sendConfigActivity(req.merged_params.uuid || fromDevice.uuid, skynet.emitToClient);
          res.json(result);
        }
      });
    });
  ...
  server.delete('/devices/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      unregister(fromDevice, req.merged_params.uuid, req.merged_params.token, skynet.emitToClient, function(err, data){
        if(err){
  ...
  server.get('/events/:uuid', function(req, res){
    ...
    //   logEvent(201, {fromUuid: fromDevice.uuid, from: fromDevice, uuid: req.merged_params.uuid});
  ...
  server.get('/authenticate/:uuid', function(req, res){
    ...
    res.setHeader('Access-Control-Allow-Origin','*');
    authDevice(req.merged_params.uuid, req.query.token, function(error, device){
      if (!device) {
        var regdata = {
          "error": {
        ...
        return res.json(regdata.error.code, {uuid:req.merged_params.uuid, authentication: false});
      }

      res.json({uuid:req.merged_params.uuid, authentication: true});
    });
  });
  ...
  server.get('/inboundsms', function(req, res){
    ...
    var data;
    try{
      data = JSON.parse(req.merged_params);
    } catch(e){
      data = req.merged_params;
    }
    var toPhone = data.To;
  ...
  server.get('/inboundyo', function(req, res){
    ...
    res.setHeader('Access-Control-Allow-Origin','*');
    var yoUsername = req.merged_params.username;
  ...
  server.post('/data/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      req.merged_params.ipAddress = req.merged_params.ipAddress || getIP(req);
      logData(req.merged_params, function(data){
        if(data.error){
          errorResponse(data.error, res);
        } else {
          ...
          // message.devices = req.merged_params.uuid;
          message.devices = "*";
  ...
  server.get('/js/meshblu.js', function(req, res){
    res.send(301, null, {
      Location: 'https://cdn.octoblu.com/js/latest/meshblu.bundle.js'
    });
  });

  server.get('/jsconsole', function(req, res){
    res.send(301, null, {
      Location: 'https://developer.octoblu.com/jsconsole'
    });
  });
  ...
  server.get('/', function(req, res){
    res.send(301, null, {
      Location: 'https://developer.octoblu.com'
    });
  });
