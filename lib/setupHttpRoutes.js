...
function setupHttpRoutes(server, skynet){
  ...
  server.put('/claimdevice/:uuid', function(req, res){
    ...
    authDevice(req.params.uuid, req.query.token, function (auth) {
      if (auth.authenticate) {
        ...
        claimDevice(auth.device, req.params, function(error, data){
          if(error){
            res.json(400, error);
          }else{
            res.json(data);
  ...
  server.put('/devices/:authUuid', function(req, res){
    res.setHeader('Access-Control-Allow-Origin','*');
    authDevice(req.params.authUuid, req.query.token, function (auth) {
      if (auth.authenticate) {
        delete req.params.authUuid;
        skynet.handleUpdate(auth.device, req.params, function(result){
          if(result.error){
  ...
  server.get('/events/:uuid', function(req, res){
    ...
    authDevice(req.params.uuid, req.query.token, function(auth){
      if (auth.authenticate){
        getEvents(req.params.uuid, function(data){
          console.log(data);
          // io.sockets.in(req.params.uuid).emit('message', data)
          if(data.error){
            res.json(data.error.code, data);
      ...
      if (auth.authenticate){
        ...
        logData(req.params, function(data){
          ...
          if(data.error){
            ...
            skynet.sendMessage(message.devices, message);
