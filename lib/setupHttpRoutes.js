...
function subscribeBroadcast(req, res, type, skynet){
  authorizeRequest(req, res, function(fromDevice){
    ...
    var uuid = req.params.uuid || req.merged_params.uuid;
...
function setupHttpRoutes(server, skynet){
  ...
  var subscribeAndForwardWithToken = function(response, uuid, token, subscriptionTypes) {
    authDevice(uuid, token, function(error, authedDevice){
      subscribeToMessageIO(response, uuid, subscriptionTypes || ['received', 'broadcast', 'sent']);
    });
  }

  var connectMessageIO = function(response) {
    var socketIOClient = SocketIOClient('ws://localhost:' + config.messageBus.port)
    response.setHeader('Connection','keep-alive');
    var readStream = new Readable();
    readStream._read = _.noop;
    readStream.pipe(response);
    socketIOClient.on('message', function(message){
  ...
  var subscribeToMessageIO = function(response, uuid, subscriptionTypes) {
    var socketIOClient = connectMessageIO(response);
  ...
  var subscribeAndForward = function(askingDevice, response, uuid, token, subscriptionTypes){
    ...
    getDevice(uuid, function(error, subscribedDevice){
      if (error) {
        return response.status(401).send({error: 'unauthorized'});
      }
      securityImpl.canReceive(askingDevice, subscribedDevice, function(error, permission){
        if (error) {
          return response.status(401).send({error: 'unauthorized'});
        }
        if (!permission) {
        ...
        if ((subscribedDevice.owner && subscribedDevice.owner === askingDevice.uuid) ||
            subscribedDevice.uuid === askingDevice.uuid) {
          newSubscriptionTypes.push('direct');
          newSubscriptionTypes.push('sent');
        }
        subscribeToMessageIO(response, uuid, subscriptionTypes || newSubscriptionTypes);
      });
    });
  }
