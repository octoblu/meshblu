...
function getAuthUuidAndToken(req) {
  ...
  if (req.authorization && req.authorization.scheme === 'Bearer') {
    var token = new Buffer(req.authorization.credentials, 'base64').toString().split(':')
    authUuid = token[0]
    authToken = token[1]
  }

  if (req.authorization && req.authorization.scheme === 'Basic') {
    authUuid = req.authorization.basic.username;
    authToken = req.authorization.basic.password;
  }
...
function setupHttpRoutes(server, skynet){
  ...
  server.del('/devices/:uuid/tokens/:token', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
  ...
  server.post('/devices', function(req, res){
    ...
    res.setHeader('Access-Control-Allow-Origin','*');
    req.params.ipAddress = req.params.ipAddress || getIP(req);
  ...
  server.post('/messages', function(req, res, next){
    try {
      req.params = _.extend(req.params, JSON.parse(req.body));
    } catch (e) {}
    authorizeRequest(req, res, function(fromDevice){
      //skynet.sendActivity(getActivity('messages',req));
      var message = {
        devices: req.params.devices,
