...
var generateSessionId = require('./generateSessionId');
...
function setupHttpRoutes(server, skynet){
  ...
  // curl -X POST http://localhost:3000/devices/:uuid/session
  server.post('/devices/:uuid/session', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('generateSessionId', req, fromDevice));

      generateSessionId(req.params.uuid, function(error, result){
        if(error){
