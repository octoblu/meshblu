...
function setupHttpRoutes(server, skynet){
  ...
  server.get('/devices', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      getDevices(fromDevice, req.query, false, function(data){
          if(data.error){
            meshbluEventEmitter.emit('devices-error', {request: req.query, fromUuid: fromDevice.uuid});
  ...
  server.get('/devices/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      getDevices(fromDevice, {uuid: req.params.uuid}, false, function(data){
          if(data.error){
            meshbluEventEmitter.emit('devices-error', {request: {uuid: req.params.uuid}, fromUuid: fromDevice.uuid});
            return errorResponse(data.error, res);
          }
  ...
  server.patch('/v2/devices/:uuid', function(req, res){
    getAuthDeviceFromRequest(req, function(error, fromDevice) {
      ...
      updateIfAuthorized(fromDevice, {uuid: req.params.uuid}, {$set: req.body}, function(error){
        if(error){
          meshbluEventEmitter.emit('update-error', {error: error.message, request: {$set: req.body}, fromUuid: fromDevice.uuid});
          return res.status(422).send({error: error.message});
        }
        meshbluEventEmitter.emit('update', {request: {$set: req.body}, fromUuid: fromDevice.uuid});
  ...
  server.put('/v2/devices/:uuid', function(req, res){
    getAuthDeviceFromRequest(req, function(error, fromDevice) {
      ...
      updateIfAuthorized(fromDevice, {uuid: req.params.uuid}, req.body, function(error){
        ...
        meshbluEventEmitter.emit('update', {request: req.body, fromUuid: fromDevice.uuid});
        return res.status(204).send();
      });
    });
  });
  ...
  server.put('/claimdevice/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      claimDevice(fromDevice, {uuid: req.params.uuid}, function(error, data){
        if(error){
          meshbluEventEmitter.emit('claimdevice-error', {request: {uuid: req.params.uuid}, fromUuid: fromDevice.uuid, fromIp: fromDevice.ipAddress});
  ...
  server.get('/devices/:uuid/publickey', function(req, res){
    getPublicKey(req.params.uuid, function(error, publicKey){
      if(error){
        meshbluEventEmitter.emit('publickey-error', {request: {uuid: req.params.uuid}, error: error.message});
        return res.send(500, {error: error.message});
      }

      meshbluEventEmitter.emit('publickey', {request: {uuid: req.params.uuid}});
  ...
  server.post('/devices/:uuid/tokens', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      generateAndStoreToken(fromDevice, req.params.uuid, function(error, result){
        if(error){
          meshbluEventEmitter.emit('generatetoken-error', {request: {uuid: req.params.uuid}, error: error, fromUuid: fromDevice.uuid});
          return errorResponse(error, res);
        }
        var uuid = req.params.uuid || fromDevice.uuid;
  ...
  server.delete('/devices/:uuid/tokens/:token', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      revokeToken(fromDevice, req.params.uuid, req.params.token, function(error, result){
        if(error){
          meshbluEventEmitter.emit('revoketoken-error', {request: {uuid: req.params.uuid}, error: error, fromUuid: fromDevice.uuid});
          return errorResponse(error, res);
        }
  ...
  server.put('/devices/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      updateFromClient(fromDevice, _.defaults({uuid: req.params.uuid}, req.merged_params), function(result){
        if(result.error){
          meshbluEventEmitter.emit('update-error', {request: req.merged_params, fromUuid: fromDevice.uuid, error: result.error});
          return errorResponse(result.error, res);
        }

        skynet.sendConfigActivity(req.params.uuid || fromDevice.uuid, skynet.emitToClient);
        meshbluEventEmitter.emit('update', {request: req.merged_params, fromUuid: fromDevice.uuid});
        res.json(result);
      });
    });
  });
  ...
  server.delete('/devices/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      unregister(fromDevice, req.params.uuid, req.merged_params.token, skynet.emitToClient, function(err, data){
        if(err){
          meshbluEventEmitter.emit('unregister-error', {request: req.merged_params, fromUuid: fromDevice.uuid, error: err});
          return errorResponse(err, res);
        }

        meshbluEventEmitter.emit('unregister', {request: req.merged_params, fromUuid: fromDevice.uuid});
        res.json(data);
      });
    });
  ...
  server.get('/mydevices', function(req, res){
    ...
    authorizeRequest(req, res, function(fromDevice){
      ...
      getDevices(fromDevice, query, true, function(data){
        if(data.error){
          meshbluEventEmitter.emit('mydevices-error', {request: query, error: data.error, fromUuid: fromDevice.uuid});
          return errorResponse(data.error, res);
        }

        meshbluEventEmitter.emit('mydevices', {request: query, fromUuid: fromDevice.uuid});
        res.json(data);
      });
    });
  });
  ...
  server.get('/authenticate/:uuid', function(req, res){
    ...
    authDevice(req.params.uuid, req.merged_params.token, function(error, device){
      if (!device) {
        ...
        meshbluEventEmitter.emit('identity-error', {request: req.merged_params, error: regdata.error.message});
  ...
  server.post('/messages', function(req, res, next){
    authorizeRequest(req, res, function(fromDevice){
      ...
      if (_.isEmpty(message.devices)) {
        meshbluEventEmitter.emit('message-error', {request: message, error: 'Invalid Message Format'});
  ...
  server.post('/data/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      saveDataIfAuthorized(skynet.sendMessage, fromDevice, req.params.uuid, req.merged_params, function(error, data){
        if(error){
          meshbluEventEmitter.emit('data-error', {request: req.merged_params, error: error, fromUuid: fromDevice.uuid});
