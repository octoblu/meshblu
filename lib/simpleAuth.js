...
module.exports  = {
  canDiscover: function(fromDevice, toDevice){
    ...
    if(fromDevice && toDevice){
      if(toDevice.owner == fromDevice.uuid){
        return true;
      }else if(toDevice.discoverWhitelist && toDevice.discoverWhitelist.length){
        viewable = false;
        ...
        return viewable;
      }else if(toDevice.discoverBlacklist && toDevice.discoverBlacklist.length){
        viewable = true;
  ...
  canSend: function(fromDevice, toDevice){
    ...
    if( toDevice && toDevice.token && message && message.token === toDevice.token ) {
      return true;
    }
