        ...
        whoAmI(data.uuid, false, function(check){
          ...
          if(!check.error){
            ...
            if(target.socketid && securityImpl.canSend(device, target)){
              ...
              //console.log('io.sockets', skynet.io);
              if(target.secure && config.tls){
                //TODO figure this out for socket.io 1.x
                //skynet.ios.sockets.socket(target.socketid).emit("bindSocket", {fromUuid: device.uuid}, bindReply);
                if(skynet.ios.sockets.connected[target.socketid]){
                  skynet.ios.sockets.connected[target.socketid].emit('bindSocket', {fromUuid: device.uuid}, bindReply);
                }
                else{
                  bindReply('target disconnected');
                }
                //socket.to(target.socketid).emit("bindSocket", {fromUuid: device.uuid}, bindReply);
              } else {
                //skynet.io.sockets.socket(target.socketid).emit("bindSocket", {fromUuid: device.uuid}, bindReply);
                //console.log(skynet.io.sockets.connected);
                if(skynet.io.sockets.connected[target.socketid]){
                  skynet.io.sockets.connected[target.socketid].emit('bindSocket', {fromUuid: device.uuid}, bindReply);
                }
                else{
                  bindReply('target disconnected');
                }
