...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('bindSocketAck', function (data) {
    if(config.broadcastActivity){
      skynet.sendActivity({ipAddress: socket.ipAddress}, function(result){
      });
    }
    ...
    getDevice(socket, function(err, device){
      whoAmI(data.uuid, false, function(check){
        if(!check.error && securityImpl.canSend(device, check)){
          if(data.payload === 'ok' || (data.payload && data.payload.result === 'ok')){
            bindSocket.connect(device.uuid, check.uuid, function(berr, val){
              if(berr){
                data.payload = berr;
              }
              skynet.socketEmit('bindSocketAck', device, data);
            });
          }else{
            skynet.socketEmit('bindSocketAck', device, data);
          }
  ...
  socket.on('bindSocket', function (data) {
    ...
    console.log('bindSocket', data);
    if(data && data.ack && config.redis){

      getDevice(socket, function(err, device){
        if(err){ return; }

        whoAmI(data.uuid, false, function(check){
          //console.log('bind whoami', check);
          if(!check.error && securityImpl.canSend(device, check)){
            skynet.socketEmit('bindSocket', check, {fromUuid: device.uuid});

          }else{
            console.log('invalid client target', device.uuid, check);
            data.payload = {error : 'invalid target'};
            return skynet.socketEmit('bindSocketAck', device, data);
          }
        });
      });
    }
        ...
        if(!message){
          ...
          bindSocket.getTarget(socket.id, function(err, target){
            if(target){
              ...
              getDevice(socket, function(err, device){
                if(device.secure && config.tls){
                  skynet.ios.in(target).send(message);

                } else {
                  //console.log('socket lookup', io.sockets.socket(target));
                  skynet.io.in(target).send(message);
                }
              });
