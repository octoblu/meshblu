...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('generateSessionId', function(message, fn){
    getDevice(socket, function(err, fromDevice){
      if(err){ return console.error(err); }
      generateSessionId(message.uuid, function(error, result){
        if(error) {
          return socket.emit('error', {method: 'generateSessionId', error: error});
        }
        fn(result);
      });
    });
