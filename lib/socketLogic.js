  ...
  socket.on('identity', function (data) {
    ...
    updateSocketId(data, function(auth){
      ...
      if (auth.status == 201){
          ...
          socketIOClient.emit('subscribe', auth.device.uuid);
          socketIOClient.emit('subscribe', auth.device.uuid + '_bc');
      } else {
  ...
  socket.on('subscribe', function(message, fn) {
      ...
      if(message.token) {
        authDevice(message.uuid, message.token, function(error, authedDevice) {
          ...
          subscribeToMessageIO(socketIOClient, message.uuid, message.types || subscriptionTypes);
          return;
        });
      }
