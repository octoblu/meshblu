...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('subscribe', function(data, fn) {
      ...
      getDevice(socket, function(err, device) {
        ...
        if(err) {
          return fn({error: "can't get device"});
        }
        ...
        whoAmI(data.uuid, false, function(results){
          ...
          securityImpl.canReceive(device, results, function(error, permission){
              if(error || !permission) {
                debug('subscribe', 'fromDevice', device, 'toDevice', results, "can't receive");
