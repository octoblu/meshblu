...
var Limiter = require('./SocketIOLimiter');
...
function socketLogic (socket, secure, skynet){
  ...
  var limiter = new Limiter(socket);
  ...
  socket.on('identity', function (data) {
    ...
    updateSocketId(data, function(auth){
      ...
      socket.messageIOClient.subscribe(auth.device.uuid, ['received']);
  ...
  socket.on('unsubscribe', function(data, fn) {
    ...
    socket.messageIOClient.unsubscribe(data.uuid, data.types || ['sent', 'received', 'broadcast']);
  ...
  socket.on('error', function() {
    console.error('error received, ignoring');
  });

  socket.on('update', limiter.throttle(function (data, fn) {
    fn = fn || _.noop
    if(!data){
      data = {};
    }
  ...
  socket.on('unregister', limiter.throttle(function (data, fn) {
    fn = fn || _.noop
    if(!data){
      data = {};
    }
  ...
  socket.on('events', limiter.throttle(function(data, fn) {
    fn = fn || _.noop
  ...
  socket.on('authenticate', limiter.throttle(function(data, fn) {
    fn = fn || _.noop
  ...
  socket.on('data', limiter.throttle(function(messageX, fn) {
    fn = fn || _.noop
  ...
  socket.on('getdata', limiter.throttle(function(data, fn) {
    fn = fn || _.noop
    skynet.throttles.query.rateLimit(socket.id, function (err, limited) {
      if(socket.throttled && limited){
        console.log('query throttled', socket.id);
  ...
  socket.on('messageAck', throttle.limit(function (data) {
    getDevice(socket, function(err, fromDevice){
  ...
  socket.on('tb', limiter.throttle(function (messageX) {
  ...
  socket.on('message', limiter.throttle(function (messageX) {
  ...
  socket.on('directText', limiter.throttle(function (messageX) {
    skynet.throttles.message.rateLimit(socket.id, function (err, limited) {
      var message = messageX;
  ...
  socket.on('getPublicKey', limiter.throttle(getPublicKey));

  socket.on('resetToken', limiter.throttle(function(message, fn){
    fn = fn || _.noop
    skynet.throttles.message.rateLimit(socket.id, function (err, limited) {
      if (socket.throttled && limited) {
        // TODO: Emit rate limit exceeded message
  ...
  socket.on('generateAndStoreToken', limiter.throttle(function(message, fn){
    getDevice(socket, function(err, fromDevice){
