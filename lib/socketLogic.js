...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('identity', function (data) {
    ...
    updateSocketId(data, function(auth){
      ...
      socket.socketIOClient = SocketIOClient('ws://localhost:' + config.messageBus.port, {'force new connection': true});
      socket.socketIOClient.id = socket.id;
      socket.socketIOClient.on('message', function(message){
        debug(socket.id, 'relay message', message, socket.skynetDevice.uuid);
        socket.emit('message', message);
      });

      socket.socketIOClient.on('config', function(message){
      ...
      subscribeToMessageIO(socket.socketIOClient, auth.device.uuid, ['received', 'broadcast']);
  ...
  socket.on('disconnect', function (data) {
    ...
    if (socket.socketIOClient) {
      socket.socketIOClient.close()
    }
  ...
  socket.on('subscribe', function(message, fn) {
      ...
      if(message.token) {
        authDevice(message.uuid, message.token, function(error, authedDevice) {
          ...
          subscribeToMessageIO(socket.socketIOClient, message.uuid, message.types || subscriptionTypes);
          return;
        });
      }
  ...
  socket.on('unsubscribe', function(data, fn) {
    fn = fn || _.noop
    socket.socketIOClient.emit('unsubscribe', data.uuid);
    socket.socketIOClient.emit('unsubscribe', data.uuid + '_bc');
    socket.socketIOClient.emit('unsubscribe', data.uuid + '_sent');
