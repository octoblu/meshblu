...
module.exports = function(fromDevice, unregisterUuid, unregisterToken, callback) {
  ...
  if(!fromDevice || !unregisterUuid){
    ...
    whoAmI(unregisterUuid, true, function(toDevice){
      if(toDevice.error){
        ...
        if((unregisterToken && toDevice.token === unregisterToken) || securityImpl.canUpdate(fromDevice, toDevice)){
          devices.remove({
            uuid: unregisterUuid
