...
module.exports = function(socket, callback) {
  ...
  if(socket.uuid && socket.token){
    ...
    if (socket.ipAddress){
      ...
      var updatedData = {socketid: socket.socketid, online: true, timestamp: moment(newTimestamp).toISOString(), ipAddress: socket.ipAddress, geo: geo, protocol: socket.protocol, secure: socket.secure}
    } else {
      var geo = null;
      var updatedData = {socketid: socket.socketid, online: true, timestamp: moment(newTimestamp).toISOString(), geo: geo, protocol: socket.protocol, secure: socket.secure}
    }
